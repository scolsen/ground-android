...
public abstract class TileEntity {
  ...
  public static Tile toTile(TileEntity tileEntity) throws MalformedURLException {
    Tile.Builder tile =
        Tile.newBuilder()
            .setId(tileEntity.getId())
            .setUrl(new URL(tileEntity.getUrl()))
  ...
  public static TileEntity create(String id, String url, String path, TileEntityState state) {
    return builder().setId(id).setUrl(url).setState(state).setPath(path).build();
  }
